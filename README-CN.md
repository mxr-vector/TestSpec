...
| 工作表     | 用途                                                     |
| ---------- | -------------------------------------------------------- |
| `功能测试` | 精简功能测试用例，包含需求 ID、步骤、预期结果、优先级和执行跟踪 |
| `性能测试` | 精简性能场景，包含基线目标、指标和执行跟踪               |

Excel 工作簿面向执行场景。测试点 ID、风险 ID、来源证据和可选测试数据等详细追溯信息保留在 `artifacts/testcases.json` 及相关 JSON 产物中，不再作为低价值列导出。
