...
TestSpec 是一个 CLI 工具，帮助 QA 团队通过结构化的工作流创建、管理和跟踪测试产物。它集成了 Claude Code、Qoder、Codex 和 Trae 等 AI 代理，以简化测试流程。

## 功能特性

- **结构化工作流**：遵循从需求到测试用例的成熟测试设计流程
- **AI 集成**：支持 Claude Code、Qoder、Codex、Trae 等多种 AI 代理
...
此命令会设置 TestSpec 工作区并配置 AI 代理集成。交互式选择默认选中 Claude Code 和 Qoder，可使用空格键选择/取消选择集成，按 Enter 确认。重新生成已选择的代理命令文件前，init 会移除所选 `.claude/commands/test/`、`.qoder/commands/test/` 或 `.trae/commands/test/` 目录中仍包含生成标记的 TestSpec 旧命令文件；不含生成标记的自定义命令文件会被保留。Codex 的 `.codex/skills/testspec-*/SKILL.md` 仅在包含生成标记或使用 `--force` 时刷新。带有生成标记的命令和 Skill 文件会被视为 TestSpec 托管文件，可能被 `testspec init` 重建。
...
| Codex       | `.codex/skills/testspec-*/SKILL.md` Skills，并附带 `AGENTS.md` 指南 |
| Qoder       | `.qoder/commands/test/*.md` 用于相同的工作流标签     |
| Trae        | `.trae/commands/test/*.md` 用于相同的工作流标签      |
...
要获得贴合需求的语义生成效果，请在 Claude Code、Codex、Qoder、Trae 或其他已配置的编码代理中运行已配置的工作流入口。Claude Code、Qoder 和 Trae 暴露 `/test:*` 命令标签；Codex 暴露 `testspec-*` Skills，并映射回相同的 `test:*` 工作流标签。代理负责读取需求文档并生成语义产物；CLI 保持无模型供应商依赖，只负责校验、导出、报告和归档。
...
`--agents` 接受 `claude`、`codex`、`qoder`、`trae`、`generic` 或 `all`；多个代理 ID 使用英文逗号分隔，例如 `--agents claude,qoder,trae`。对 `testspec init` 使用 `--force` 会刷新已有的 TestSpec 生成代理命令文件；对 `testspec new` 使用 `--force` 会覆盖已有测试变更工作区。
...
├── .codex/skills/testspec-*/ # Codex Skills（如果选择了此集成）
├── .qoder/commands/test/     # Qoder 命令（如果选择了此集成）
├── .trae/commands/test/      # Trae 命令（如果选择了此集成）
├── AGENTS.md                 # Codex 或通用代理指南（如果选择了此集成）
└── testspec/
    ├── changes/
    │   ├── login-v2/         # 活跃的测试变更
    │   │   ├── proposal.md              # 测试提案
...
**Qoder / Trae:**
使用 `.qoder/commands/test/*.md` 或 `.trae/commands/test/*.md` 中相同的 `/test:*` 工作流标签。这些命令文件会在正文中包含简短说明、用户参数和与供应商无关的 prompt-pack 规则。

**Codex:**
使用 `.codex/skills/testspec-*/SKILL.md` 中生成的 Skills，例如 `testspec-analysis`、`testspec-points` 和 `testspec-excel`。每个 Skill 都会映射到对应的 `test:*` 工作流标签，并包含底层 CLI 命令和与供应商无关的 prompt-pack 规则。`AGENTS.md` 仍作为全局说明。
