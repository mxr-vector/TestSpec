...
此命令会设置 TestSpec 工作区并配置 AI 代理集成。交互式选择默认选中 Claude Code 和 Qoder，可使用空格键选择/取消选择集成，按 Enter 确认。重新生成已选择的代理命令文件前，init 会移除所选 `.claude/commands/test/`、`.qoder/commands/test/` 或 `.trae/commands/test/` 目录中仍包含生成标记的 TestSpec 旧命令文件；不含生成标记的自定义命令文件会被保留。带有生成标记的命令文件会被视为 TestSpec 托管文件，可能被 `testspec init` 删除并重建。
...
要获得贴合需求的语义生成效果，请在 Claude Code、Codex、Qoder、Trae 或其他已配置的编码代理中运行 `test:*` 标签。代理负责读取需求文档并生成语义产物；CLI 保持无模型供应商依赖，只负责校验、导出、报告和归档。
...
| `性能测试` | 详细性能场景，包含场景编号、关联测试点、数据规模、负载模型、目标指标、监控观察点和执行状态 |
