...
import { EXPORT_FILE_SUFFIXES } from "../core/config.js";
...
export function registerWorkflowCommands(program: Command): void {
   ...
   * - 性能测试表：场景编号、业务模块、场景名称、性能测试类型、关联测试点、测试目标、数据规模、负载模型、目标指标、实际指标、监控观察点、执行结果
   *
   * @param {string} [name] - 测试变更名称，如果不指定则自动检测唯一的工作区
   */
  program
  ...
  program
    ...
    .action(async (name?: string) => {
      ...
      const outputPath = join(
        workspace.artifactsDir,
        `${workspace.name}${EXPORT_FILE_SUFFIXES.xmindCases}`
      );
