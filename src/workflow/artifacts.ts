...
export async function writeProposal(
  ...
  const outputPath = join(workspace.changeDir, WORKFLOW_FILES.proposal);
...
export async function writeRequirementsAnalysis(workspace: ChangeWorkspace): Promise<string> {
  ...
  const outputPath = join(workspace.changeDir, WORKFLOW_FILES.requirementsAnalysis);
...
export async function writeTestPoints(workspace: ChangeWorkspace): Promise<string> {
  ...
  const outputPath = join(workspace.specsDir, WORKFLOW_FILES.testpoints);
  ...
  await writeFile(
    ...
    [
      ...
      "## 上下文来源",
      "",
      `- 测试提案：${proposal.trim().length > 0 ? WORKFLOW_FILES.proposal : "未生成"}`,
      `- 需求分析：${analysis.trim().length > 0 ? WORKFLOW_FILES.requirementsAnalysis : "未生成"}`,
      "",
      "## 核心流程",
      "",
